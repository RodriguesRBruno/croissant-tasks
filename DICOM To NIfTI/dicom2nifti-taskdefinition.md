### Assumptions 
- This Croissant Task Definition refers to the general problem of converting a Medical image from DICOM format (.dcm) to NIfTI format (.nii.gz).
...
- The `sample_data` field also includes concrete data, in the form of `sample_input` and `sample_output`. In a real use case, this should be compose of small dataset(s) that may be used for other developers while building their solution for testing, validation, etc.
  - This sample data is used to implement and debug the corresponding solutions.
- Solutions must satisfy the following conditions:
  - Inputs and outputs should conform to the specifications defined in this Task Definition.
  - The `sample_data.inputs` must generate `sample_data.outputs` when applied to any given solution.
...
```json
{
    "task_definition": {
        ...
        "description": "This Task Definition defines the generalized task of converting medical images from the DICOM Format (.dcm) to NIfTI (.nii.gz)",
        "url": "https://url/for/task/webpage",
